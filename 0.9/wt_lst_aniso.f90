...
REAL*8, DIMENSION(:,:,:,:,:,:,:), ALLOCATABLE :: QzpLLSd
INTEGER, DIMENSION(:,:,:,:,:,:,:), ALLOCATABLE :: IQzpLLSd
INTEGER, DIMENSION(:,:,:,:,:,:,:), ALLOCATABLE :: IQzpdifLLSd
 ...
 CASE("S")
  ...
  ELSE
   ...
   IF (RatioNki>EpsMin) THEN
    ...
    Aux4= RatioNke*Ui/SQRT(Aux1i)/Aux2i**(Kappai+Alphai-1.)
...
ALLOCATE (QzpLLSd(nqx,nqz,nqx,-1:1,-1:1,-1:1,-1:1))
ALLOCATE (IQzpLLSd(nqx,nqz,nqx,-1:1,-1:1,-1:1,-1:1))
ALLOCATE (IQzpdifLLSd(nqx,nqz,nqx,-1:1,-1:1,-1:1,-1:1))
QxpLLSd= 0.
IQxpLLSd= 0
IQxpdifLLSd= 0
QzpLLSd= 0.
     ...
     DO S2= -1,1,2
      ...
      IF (Qz>Qx) THEN
       DO iqxp= 1,nqx
        ...
        X1 = 0.
        X2= 1.0
        CALL ZBRAC2(Funcz_RcdLLS,X1,X2,Succes)
        IF (Succes .eqv. .true.) THEN
         Qzp= RTBIS(Funcz_RcdLLS,X1,X2,Xacc)
        ELSE
         Qzp= 0.
        END IF
        IF(Qzp>0.) THEN
         Qzdif= Qz-S2*Qzp
         CALL Locate(VQz,nqz,Qzp,kres)
         IQzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= kres
         QzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= Qzp
         CALL Locate(VQz,nqz,ABS(Qzdif),kres)
         IQzpdifLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= kres
        ELSE
         IQzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= -1
         QzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= 0.D0
         IQzpdifLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)= -1
        END IF
       END DO
      ELSE
       DO kqzp= 1,nqz
...
REAL*8 :: AuxSE,D1,AuxCoef, D1Real, D1Aprox, D1Zerado, D1AproxNoS, D1AproxAmbos
REAL*8 :: AuxScatElSpo
!REAL*8 :: AuxScatElInd
...
INTEGER :: iqx,kqz,iqxp,kqzp
INTEGER :: iresp,iresm
INTEGER :: ires,iresdif,iresdif2,kres,kresdif,kresdif2
INTEGER :: S1,S2,S3,SS
INTEGER :: Asp,Aspp
CHARACTER(LEN=1) :: WaveT
...
!TODO
  ...
  IF(LdecayLS=="Yes") THEN
   ...
   EcalS(:,:,+1)= ISp(:,:)
   IF (Qz>Qx) THEN
    VauxqxA= 0.   ! Initializes the integrand of the Qxp integrals
    VauxqxB= 0.
    DO iqxp= 1,nqx
     Qxp= VQx(iqxp)
     Qxp2= Qxp**2
     DO sigmap= -1,1,2
      Asp= sigmap/sigma
      Vauxqzp(:)= EcalL(iqxp,:,sigmap)
      DO sigmapp= -1,1,2
       Aspp= sigmapp/sigma
       DO S1= -1,1,2
        DO S2= -1,1,2
         Qzp= QzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)
         kres= IQzpLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)
         kresdif= IQzpdifLLSd(iqx,kqz,iqxp,Asp,Aspp,S1,S2)
         Qzp2= Qzp**2
         Qxdif= Qx-S1*Qxp
         Qzdif= Qz-S2*Qzp
         ...
         D1= -S2*sigmap*1.5D0*VRTeTs(m)*Qzp/Zlqp &
          + S2*S3*sigmapp*AA*SQRT(VRTeTs(m))*Qzdif &
          /SQRT(Qxdif**2+Qzdif**2+EpsMin) &
          /(1.D0+Beta*(Qxdif**2+Qzdif**2)/2.D0)**(1.5)
         AuxCoef= SQRT((Qxdif)**2+(Qzdif)**2)*(S1*Qx*Qxp+S2*Qz*Qzp)**2 &
          /(Qxp2+Qzp2+EpsMin)/(ABS(D1)+EpsMin)
         iresdif2= MAX(ABS(iqx-S1*iqxp),1)
         !Vauxqxdif(:)= EcalS(:,ABS(kresdif2+S2*SIGN(1,kresdif2)),sigmapp)
         CALL Aux_Coef_Decay_x(nqxa,nqza,sigmapp,kres,kresdif,iresdif2,&
          Qxp,Qzp,Qxdif,Qzdif,"L","S",VQxa,VQza,Vauxqzp,EcalS,&
          Iqp,Iqdif)
         VauxqxA(iqxp)= VauxqxA(iqxp)+AuxCoef*sigma*Zlq*Iqp*Iqdif
         VauxqxB(iqxp)= VauxqxB(iqxp)-AuxCoef*(S2*sigmap*Zlqp*Iqdif &
     ...
     END DO
    END DO
    CALL Simpson(VQx,VauxqxA,nqx,Aux)
    CoefLLSdA= Aux
    CALL Simpson(VQx,VauxqxB,nqx,Aux)
    CoefLLSdB= Aux
   ELSE
      ...
      DO sigmapp= -1,1,2
       ...
       DO S1= -1,1,2
        DO S2= -1,1,2
         ...
         D1= -S2*sigmap*1.5D0*VRTeTs(m)*Qxp/Zlqp &
         ...
         !  /SQRT(Qxdif**2+Qzdif**2+EpsMin)
          ...
          ! D1AproxAmbos= -S2*sigmap*(1.5*VRTeTs(m)/SQRT(VRNeNs(m))*Qxp) &
          ! + S1*S3*sigmapp*AA*SQRT(VRTeTs(m))*Qxdif &
          ! /SQRT(Qxdif**2+Qzdif**2+EpsMin)
         ...
         AuxCoef= SQRT((Qxdif)**2+(Qzdif)**2)*(S1*Qx*Qxp+S2*Qz*Qzp)**2 &
          /(Qxp2+Qzp2+EpsMin)/(ABS(D1)+EpsMin)
         kresdif2= MAX(ABS(kqz-S2*kqzp),1)
         !Vauxqxdif(:)= EcalS(:,ABS(kresdif2+S2*SIGN(1,kresdif2)),sigmapp)
         CALL Aux_Coef_Decay_z(nqxa,nqza,sigmapp,ires,iresdif,kresdif2,&
...
!   END IF
  ...
  IF(LscatLL=="Yes") THEN
   ...
   EcalL(:,:,-1)= ILm(:,:)
   EcalL(:,:,+1)= ILp(:,:)
   ! Electron contribution, Maxw+Kappa:
   VintqxA= 0.D0
   VintqxB= 0.D0
   DO i= 1,nqx
    Qxp= VQx(i)
     ...
     DO sigmap= -1,1,2
      DO S1= -1,1,2
       ...
       ELSE
        ...
        Aux3m= Aux0 * (1.D0-RatioNke)*EXP(-Aux/VRTeTs(m)) &
         /SQRT(VRTeTs(m))
        Aux4m= Aux3m * 2.D0/VRTeTs(m)* (sigma*Zlq-sigma*Zlqp) 
        Auxe= Aux3m
        Auxe2= Aux4m 
        IF ( RatioNke>EpsMin ) THEN
         Aux1= Kappae*Uek2
         Aux2= 1.D0+Aux/Aux1
         Aux3= Aux0 * RatioNke/SQRT(Aux1) &
          /Aux2**(Kappae+Alphae-0.5D0)
         Aux4= Aux3 * 2.D0/Aux1/Aux2
         Auxe= Auxe + Aux3 * RatioGammaem051 
         Auxe2= Auxe2 + Aux4*RatioGammaep051*(sigma*Zlq-sigma*Zlqp)
        ELSE
        END IF
       END IF
       !Change here                   AuxScatElSpo
       VintqzA(k)= VintqzA(k) + (Auxe*0.0)*(sigma*Zlq) &
        * (AuxSE*Geff*(sigma*Zlq)*EcalL(i,k,sigmap))
       VintqzB(k)= VintqzB(k) &
       !Change here V AuxScatElSpo
        + (Auxe*0.0)*(AuxSE*Geff*(sigma*Zlq)*(-sigma*Zlqp)) &
        + Auxe2*(sigma*Zlq-sigma*Zlqp)*EcalL(i,k,sigmap)*0.0
        !Also change here                                 ^AuxScatElInd
      END DO
     END DO
    END DO
    ...
    VintqxB(i)= Aux
   END DO
   AuxCoef= 1.D0/SQRT(Pi)/VRNeNs(m)/Q2
   CALL Simpson(VQx,VintqxA,nqx,Aux)
   CoefLLLsA= AuxCoef * Aux
   CALL Simpson(VQx,VintqxB,nqx,Aux)
   CoefLLLsB= AuxCoef * Aux
    ...
    DO sigmap= -1,1,2
     DO S1= -1,1,2
      ...
      DO SS= -1,1,2
       ...
       END IF
       IF (sigmap==1) THEN
        VintPhipB(i)= VintPhipB(i)+Aux*Iqpp * SS*Qs/Qstar
       ELSE
        VintPhipB(i)= VintPhipB(i)+Aux*Iqpm * SS*Qs/Qstar
       END IF
      END DO !SS
     END DO !S1
    END DO !sigmap
...
!   END IF
  ...
  IF(TscatLT=="Yes") THEN
   ...
   CoefTLTsA= CoefTLTsA &
    ...
    + (2./3.)/VRNeNs(m) * (Ztq)**3 * 0.0*AuxSE*Geff*Aux * (1.-RatioNke)
    !Change here                     ^AuxScatElSpo
   CALL Simpson(VPhip,VintPhipB,nph,Aux)
   CoefTLTsB= CoefTLTsB &
    - (2./3.)/VRNeNs(m) * (Ztq)**3 * AuxSE*Geff*Aux/2. * (1.-RatioNki) &
    - (2./3.)/VRNeNs(m) * (Ztq)**3 * 0.0*AuxSE*Geff*Aux/2. &
    * (1.-RatioNke)
    !Change here                     ^AuxScatElSpo
   ! Induced scattering due to the ions:
   VintPhipB= 0.
...
SUBROUTINE Aux_Coef_Decay_z(nqxa,nqza,sigmapp,ires,iresdif,kresdif2,&
...
USE Phys_Constants
IMPLICIT NONE
INTEGER, INTENT(in) :: nqxa,nqza,sigmapp,ires,iresdif,kresdif2
...
ELSE
 SELECT CASE(iresdif)
  CASE(-1)   ! Qxdif<0
   Iqdif= 0.
  CASE(0)
   SELECT CASE(kresdif2)
    CASE(-1)   ! Qzdif<0
     Iqdif= 0.
    CASE(0)
     Iqdif= EcalAux(1,1,sigmapp)
    CASE(1:)
     IF (kresdif2<nqza) THEN
      Iqdif= EcalAux(1,kresdif2,sigmapp)
     ELSE
      HBrL=0. ! The new effects are not relevant for large Q and are neglected
      HBrS=0.
      HGcL=0.
    ...
    CASE DEFAULT
    OPEN(98,FILE='Warning_Select_Case_Aux.wt')
    WRITE(98,*) ' kresdif2= ',kresdif2
    WRITE(98,*) ' kresdif2 must be integer greater or equal -1 !!'
    CLOSE(98)
   END SELECT
  CASE(1:)   ! iresdif >= 1
   IF (iresdif<nqxa) THEN
    SELECT CASE(kresdif2)
     ...
     CASE(1:)
      IF (kresdif2<nqza) THEN
       Vauxqxdif(:)= EcalAux(:,kresdif2,sigmapp)
       CALL Aitp1d2(nqxa,VQxa,Vauxqxdif,ABS(Qxdif),Iqdif,iresdif)
      ELSE
       HBrL=0. ! The new effects are not relevant for large Q and are neglected
       HBrS=0.
...
SUBROUTINE Aux_Coef_Decay_x(nqxa,nqza,sigmapp,kres,kresdif,iresdif2,&
  Qxp,Qzp,Qxdif,Qzdif,Wavep,Wavedif,VQxa,VQza,Vauxqzp,EcalAux,&
  Iqp,Iqdif)
USE Common_Params
USE Common_Arrays
USE Math_Constants
USE Phys_Constants
IMPLICIT NONE
INTEGER, INTENT(in) :: nqxa,nqza,sigmapp,kres,kresdif,iresdif2
REAL*8, INTENT(in) :: Qxp,Qzp,Qxdif,Qzdif
REAL*8, INTENT(out) :: Iqp,Iqdif
REAL*8 :: Iinit
REAL*8 :: HBrL,HBrS,HGcL,HGcS
...
REAL*8, DIMENSION(nqxa), INTENT(in) :: VQxa
REAL*8, DIMENSION(nqza), INTENT(in) :: VQza
REAL*8, DIMENSION(nqza), INTENT(in) :: Vauxqzp
REAL*8, DIMENSION(nqza) :: Vauxqzdif
REAL*8, DIMENSION(nqxa,nqza,-1:1), INTENT(in) :: EcalAux
CHARACTER(LEN=1), INTENT(in) :: Wavep,Wavedif

Aux= VQxa(1)   ! Only to avoid a warning message at compilation.
...
SELECT CASE(kres)
 ...
 CASE(1:)   ! kres >= 1
  IF (kres<nqza) THEN
   CALL Aitp1d2(nqza,VQza,Vauxqzp,Qzp,Iqp,kres)
  ELSE
   HBrL=0. ! The new effects are not relevant for large Q and are neglected
   HBrS=0.
   HGcL=0.
 ...
 OPEN(98,FILE='Warning_Select_Case_Aux.wt')
 WRITE(98,*) ' kres= ',kres
 WRITE(98,*) ' kres must be integer greater or equal -1 !!'
 CLOSE(98)
END SELECT
...
ELSE
 SELECT CASE(kresdif)
  ...
  CASE(0)
   SELECT CASE(iresdif2)
    ...
    CASE(0)
     Iqdif= EcalAux(1,1,sigmapp)
    CASE(1:)
     IF (iresdif2<nqxa) THEN
      Iqdif= EcalAux(iresdif2,1,sigmapp)
     ELSE
      HBrL=0. ! The new effects are not relevant for large Q and are neglected
      HBrS=0.
      HGcL=0.
      HGcS=0.
      CALL Iwave_Init(ABS(Qxdif),VQz(1),Iinit,Wavedif,HBrL,HBrS,HGcL,HGcS)
      Iqdif= Iinit
     END IF
    CASE DEFAULT
    OPEN(98,FILE='Warning_Select_Case_Aux.wt')
   ...
   END SELECT
  CASE(1:)   ! kresdif >= 1
   IF (kresdif<nqza) THEN
    SELECT CASE(iresdif2)
     CASE(-1)   ! Qxdif<0
      Iqdif= 0.
     CASE(0)
      Vauxqzdif(:)= EcalAux(1,:,sigmapp)
      CALL Aitp1d2(nqza,VQza,Vauxqzdif,ABS(Qzdif),Iqdif,kresdif)
     CASE(1:)
      IF (iresdif2<nqxa) THEN
       Vauxqzdif(:)= EcalAux(iresdif2,:,sigmapp)
       CALL Aitp1d2(nqza,VQza,Vauxqzdif,ABS(Qzdif),Iqdif,kresdif)
      ELSE
       HBrL=0. ! The new effects are not relevant for large Q and are neglected
       HBrS=0.
  ...
  CASE DEFAULT
  OPEN(98,FILE='Warning_Select_Case_Aux.wt')
  WRITE(98,*) ' kresdif= ',kresdif
  WRITE(98,*) ' kresdif must be integer greater or equal -1 !!'
  CLOSE(98)
 END SELECT
END IF

RETURN
END SUBROUTINE Aux_Coef_Decay_x
...
! Funcx_RcdLLS= ZLAprox(Qx,ABS(Qz))-Asp*S2*ZLAprox(Qxp,ABS(Qzp)) - Aspp*S3*ZS(Qxdif,ABS(Qzdif))
...
REAL*8 FUNCTION Funcz_RcdLLS(Qzp)
USE Common_Params
USE Common_Arrays
USE Math_Constants
IMPLICIT NONE
REAL*8, INTENT(in) :: Qzp
REAL*8 :: Qx,Qz,Qxp,Qxdif,Qzdif,Beta
INTEGER :: Asp,Aspp,S1,S2,S3 
INTEGER :: m
...
Funcz_RcdLLS= ZL(Qx,ABS(Qz))-Asp*S2*ZL(Qxp,ABS(Qzp)) &
    -Aspp*S3*ZS(Qxdif,ABS(Qzdif))
RETURN
END FUNCTION Funcz_RcdLLS
...
! 	DO k=1, nuz
! 		DO i=1, nux
! 			IF ( ABS(Fe(i,k)-FeOld(i,k)) > ABS(DeltaFe(i,k)) ) THEN
! 				IF (Fe(i,k)-FeOld(i,k)>0) THEN
! 					Fe(i,k) = FeOld(i,k) + 1.05*ABS(DeltaFe(i,k))
! 				ELSE
! 					Fe(i,k) = FeOld(i,k) - 1.05*ABS(DeltaFe(i,k))
! 				END IF
! 			END IF
